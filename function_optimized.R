...
###' functions 
      ...
      # this function returns a vector of the names of the .rds files in the specified study directory
      # with the file extensions stripped
      datavector <- function(studyid) {
        datavector <- gsub("\\.rds$", "", list.files(path = str_c(prt_directory, studyid, "/"), pattern = "\\.rds$"))
        return(datavector)
      }

      # this function reads in the dataset stored in the specified .rds file
      read.rds <- function(studyid, ds) readRDS(str_c(prt_directory, studyid, "/", ds, ".rds"))
      ...
      # this function checks if the specified dataset has the USUBJID variable in it
      # if present, returns the dataset intact else, returns NULL
      dt_fltr <- function(ds,studyid) {
        df <- read.rds(studyid,ds)
        if ("USUBJID" %in% names(df)) {
          return(df)
        } else {}
      }
      
      # this function returns a list of the datasets in the specified study directory
